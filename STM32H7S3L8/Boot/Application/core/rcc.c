...
static void rcc_setup_pll( void )
{
    ...
    WRITE_REG( RCC->PLLCKSELR,
             ...
             | 0x06 << RCC_PLLCKSELR_DIVM3_Pos );       /* DIVM3 = 24MHz / 6 = 4MHz */
    ...
    MODIFY_REG( RCC->PLLCFGR,
                ...
                RCC_PLLCFGR_PLL3VCOSEL_Msk                      /* VCO = High (384MHz..1672MHz, ref2_ck = 2MHz..16MHz) */
              | 0x01 << RCC_PLLCFGR_PLL3RGE_Pos );              /* RGE = 2MHz..4MHz */

    /* Настроить Fractional */
    CLEAR_BIT( RCC->PLLCFGR, RCC_PLLCFGR_PLL3FRACEN_Msk );

    CLEAR_REG( RCC->PLL3FRACR );                                /* Fractional = 0 */
    ...
    MODIFY_REG( RCC->PLL3DIVR1,
                ...
                ( 100 - 1 ) << RCC_PLL3DIVR1_DIVN_Pos );        /* DIVN = 4MHz * 100 = 400MHz */

    /* Включить и настроить DIVS */
    SET_BIT( RCC->PLLCFGR, RCC_PLLCFGR_PLL3SEN_Msk );

    MODIFY_REG( RCC->PLL3DIVR2,
                RCC_PLL3DIVR2_DIVS_Msk,
                ( 8 - 1 ) << RCC_PLL3DIVR2_DIVS_Pos );          /* DIVS = 400MHz / 8 = 50MHz */
